...
namespace SSLKey
{
    ...
    public class Win32
    {
        ...
        public static UIntPtr RSA_CSP_PUBLICKEYBLOB = new UIntPtr(19);

        public static uint ENCODING_TYPE = PKCS_7_ASN_ENCODING | X509_ASN_ENCODING;
    ...
    public class opensslkey
    {
        ...
        public static Boolean getModulusExponentPrivateKeyInfo(byte[] encpkcs8, SecureString secpswd, out Byte[] Modulus, out Byte[] Exponent)
        {
            Modulus = new byte[0];
            Exponent = new byte[0];
            ...
            byte[] OIDdesCBC = { 0x06, 0x05, 0x2b, 0x0e, 0x03, 0x02, 0x07 }; //lo agrege para pruebas con DES
            ...
            try
            {
                ...
                Boolean esDes = true;
                //probar primero con DES
                seqdes = binr.ReadBytes(7);		//read the Sequence OID
                if (!CompareBytearrays(seqdes, OIDdesCBC))
                {
                    esDes = false;
                    //ahora probar con TRIPLEDES
                    seqdes = AgregarBytes(seqdes, binr.ReadBytes(3));
                ...
                pkcs8 = DecryptPBDK2(encryptedpkcs8, salt, IV, secpswd, iterations, esDes);
                if (pkcs8 == null)	// probably a bad pswd entered.
                {
                    throw new Exception("La contraseña del archivo .key es incorrecta");
                    return false;
                }
                ...
                seq = new byte[15];
                // ---------  Set up stream to read the asn.1 encoded SubjectPublicKeyInfo blob  ------
                mem = new MemoryStream(pkcs8);
                lenstream = (int)mem.Length;
                ...
                try
                {
                    ...
                    byte[] rsaprivkey = binr.ReadBytes((int)(lenstream - mem.Position));
                    //RSACryptoServiceProvider rsa = DecodeRSAPrivateKey(rsaprivkey);


                    // ---------  Set up stream to decode the asn.1 encoded RSA private key  ------
                    mem = new MemoryStream(rsaprivkey);
                    binr = new BinaryReader(mem); //wrap Memory Stream with BinaryReader for easy reading
                    bt = 0;
                    twobytes = 0;
                    int elems = 0;
        ...
        public static Boolean DecodeEncryptedPrivateKeyInfo(byte[] encpkcs8, SecureString secpswd, out RSACryptoServiceProvider RSA, out String Errores)
        {
            ...
            try
            {
                ...
                if (pkcs8 == null)	// probably a bad pswd entered.
                {
                    Errores = "El archivo la contraseña no corresponde al archivo .KEY";
                    return false;
                }
            ...
            {
                Errores = "Error generando RSA " + e.Message;
                return false;
            }
        ...
        private static byte[] AgregarBytes(byte[] Original, byte[] Agregar)
        {
            byte[] Resultado = new byte[Original.Length + Agregar.Length];

            int X = 0;
            for (X = 0; Original.Length > X; X++)
                Resultado[X] = Original[X];
            for (int Y = 0; Agregar.Length > Y; Y++)
                Resultado[X + Y] = Agregar[Y];
        ...
        public static byte[] DecryptPBDK2(byte[] edata, byte[] salt, byte[] IV, SecureString secpswd, int iterations, Boolean esDES)
        {
            ...
            try
            {
                ...
                if (esDES)
                {
                    DES decAlg = DES.Create();
                    decAlg.Key = kd.GetBytes(8);
                    decAlg.IV = IV;
        ...
        private static Byte[] IngresarByte(Byte[] Original)
        {
            int Total = 64;
            if (Original.Length > 70)
                Total = 128;
